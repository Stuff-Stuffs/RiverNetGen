...
public class GeoColumnInterpolator3d {
    ...
    public int interpolate(final double x, final double y, final double z) {
        double[] bestScore = new double[]{Double.NEGATIVE_INFINITY};
        int best = search(x,y,z, fResult.yScale, fResult.sections, bestScore);
        int chosen = 0;
        int r = search(x,y,z, sResult.yScale, sResult.sections, bestScore);
        if(r!=-1) {
            best = r;
            chosen = 1;
        }
        r = search(x,y,z, tResult.yScale, tResult.sections, bestScore);
        if(r!=-1) {
            best = r;
            chosen = 2;
        }
    ...
    private static int search(double x, double y, double z,double yScale, Section[] sections, double[] bestScoreArr) {
        double bestScore = bestScoreArr[0];
        int best = -1;
        for (int i = 0; i < sections.length; i++) {
            final Section section = sections[i];
            final double dx = x - section.x;
            final double dy = 2 * (y * yScale - section.y);
