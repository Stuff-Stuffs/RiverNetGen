...
public class GeoTest {
    public static void main(final String[] args) {
        ...
        final DikeGeoFeature<String> feature = new DikeGeoFeature<>(0, "rock", 200, 400, 0, 1, -1, 0, 7, 500);
        final List<String> rocks = List.of("base0", "base1", "base2", "base3", "base4", "base5", "base6", "base7");
        final GeoFeatureApplicator<String> applicator = GeoFeatureApplicator.create((ap, registry) -> {
            final Tri.Cache<GeoColumnInterpolator3d> interpolators = new Tri.Cache<>(8, coordinate -> {
                ...
                return new GeoColumnInterpolator3d(corners[0].x(), corners[0].y(), random(corners[0], layerCount, rocks, seed, registry), corners[1].x(), corners[1].y(), random(corners[1], layerCount, rocks, seed, registry), corners[2].x(), corners[2].y(), random(corners[2], layerCount, rocks, seed, registry));
            });
            return context -> interpolators.get(Tri.fromCartesian(context.x() * scale, context.z() * scale)).interpolate(context.x() * scale, context.y(), context.z() * scale);
        }, 4);
    ...
    private static GeoColumn random(final Tri.Coordinate coordinate, final int layerCount, final List<String> rocks, final int seed, final GeoFeature.Registry<String> registry) {
        return random(HashCommon.murmurHash3(coordinate.a() ^ HashCommon.murmurHash3(coordinate.b() ^ HashCommon.murmurHash3(coordinate.c() + seed)) + seed + seed), layerCount, rocks, registry);
    }

    private static GeoColumn random(final int seed, final int layerCount, final List<String> rocks, GeoFeature.Registry<String> registry) {
        ...
        for (int i = 0; i < layerCount; i++) {
            data[i] = registry.getGeoId(rocks.get((randomState & 0x7FFF_FFFF) % count));
